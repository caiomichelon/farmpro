...
import { CATTLE_LOT_COST_CATEGORY_LABELS, useCattleLotCosts } from '../../../../../../../src/hooks/useCattleLotCosts';
import type { CattleLotCost, CattleLotCostCategory } from '../../../../../../../src/types/database';
import { radius, spacing, typography, useColors, type Colors } from '../../../../../../../src/theme';

const CATEGORY_OPTIONS = Object.entries(CATTLE_LOT_COST_CATEGORY_LABELS).map(([value, label]) => ({
  value: value as CattleLotCostCategory,
  label,
}));

export default function CattleLotCostsScreen() {
  const colors = useColors();
  const styles = useMemo(() => createStyles(colors), [colors]);
  const { lotId } = useLocalSearchParams<{ lotId: string }>();
  const { costs, totalCost, isLoading, error, createCost } = useCattleLotCosts(lotId);
...
function CostRow({ cost, styles }: { cost: CattleLotCost; styles: ReturnType<typeof createStyles> }) {
  return (
    <Card style={styles.card}>
      ...
      <View style={styles.cardMetaRow}>
        <View style={styles.categoryBadge}>
          <Text style={styles.categoryBadgeText}>{CATTLE_LOT_COST_CATEGORY_LABELS[cost.category]}</Text>
        </View>
...
function NewCostForm({
  ...
  onCancel: () => void;
  onCreate: (values: { category: CattleLotCostCategory; description: string; amount: number }) => Promise<string | null>;
}) {
  const colors = useColors();
  const styles = useMemo(() => createStyles(colors), [colors]);
  const [category, setCategory] = useState<CattleLotCostCategory>('racao');
  ...
  return (
    <View style={styles.form}>
      ...
      <TextField label="Descrição" value={description} onChangeText={setDescription} placeholder="Ex.: Ração — 40 sacas" />
