 ...
 * custo lançado (Lavoura + Corte). Extraído pra hook compartilhado porque
 * o cofrinho da meta usa exatamente o mesmo número. */
export function useConsolidatedResult(farmId: string | undefined) {
  ...
  const isLoading = lotsLoading || seasonsLoading || revenueLoading;
  ...
  const totalCost = lavouraCost + corteCost;
