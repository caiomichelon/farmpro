 ...
 * meta). Soma tanto vendas amarradas a safra quanto lançadas soltas, direto
 * na fazenda (sem talhão). */
export function useGrainRevenue(farmId: string | undefined) {
  ...
  useEffect(() => {
    ...
    ]).then(([bySeason, direct]) => {
      if (cancelled) return;
      const rows = [...(bySeason.data ?? []), ...(direct.data ?? [])];
      const total = rows.reduce((sum, r) => sum + Number(r.quantity_sacas) * Number(r.price_per_saca), 0);
      setRevenue(total);
      setIsLoading(false);
    });
    return () => {
