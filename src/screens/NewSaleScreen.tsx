...
/** Formulário de nova venda — amarrada a uma safra (seasonId) ou solta
 * direto na fazenda (sem seasonId), espelhando o HarvestScreen. */
export function NewSaleScreen({
  ...
  prefillTruckPlate?: string;
}) {
  const colors = useColors();
  const styles = useMemo(() => createStyles(colors), [colors]);
  const t = useT();
  const { buyers, createBuyer } = useGrainBuyers(farmId);
  const { createSale } = useGrainSales({ seasonId, farmId });
