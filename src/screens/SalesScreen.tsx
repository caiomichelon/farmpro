...
/** Tela de Vendas — reusada amarrada a uma safra (seasonId) ou solta direto
 * na fazenda (sem seasonId), espelhando o HarvestScreen. */
export function SalesScreen({ farmId, seasonId }: { farmId: string; seasonId?: string }) {
  const colors = useColors();
  const styles = useMemo(() => createStyles(colors), [colors]);
  const t = useT();
  const { sales, totalSacasSold, totalValue, totalFreight, netValue, isLoading, error, reload } = useGrainSales({
    seasonId,
    farmId,
  });
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      {isLoading ? (
        ...
        <ScrollView contentContainerStyle={styles.content}>
          ...
          <Button label={t('sales.newSale')} onPress={() => router.push(`${basePath}/nova-venda`)} />
