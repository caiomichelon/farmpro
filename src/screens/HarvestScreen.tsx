...
import { useGrainSales } from '../hooks/useGrainSales';
...
/** Tela de Colheita — reusada tanto amarrada a uma safra específica
 * (seasonId, pra quem organiza por talhão) quanto solta direto na fazenda
 * (sem seasonId, pra quem só quer lançar as notas sem se preocupar com
 * talhão/safra). */
export function HarvestScreen({ farmId, seasonId }: { farmId: string; seasonId?: string }) {
  ...
  const { sales, totalSacasSold, totalValue, createSale, reload: reloadSales } = useGrainSales({ seasonId, farmId });
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <FadeSlideIn delay={40}>
          <Pressable
            ...
            onPress={() => router.push(`${basePath}/vendas`)}
          >
            <View style={{ flex: 1 }}>
            ...
            {isAdding ? (
              ...
              <>
                ...
                <Button
                  ...
                  variant="ghost"
                  onPress={() => router.push(`${basePath}/colheita-importar`)}
                />
              </>
            )}
